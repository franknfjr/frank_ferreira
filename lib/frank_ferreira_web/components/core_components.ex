defmodule FrankFerreiraWeb.CoreComponents do
  @moduledoc """
          ...
          <div class="w-full max-w-3xl p-4 sm:p-6 lg:py-8">
            ...
            >
              <div class="absolute top-6 right-5">
                ...
                >
                  <.icon name="fa-x-mark-solid" class="h-5 w-5" />
                </button>
              </div>
              <div id={"#{@id}-content"}>
  ...
  def flash(assigns) do
    ...
    >
      <p :if={@title} class="flex items-center gap-1.5 text-sm font-semibold leading-6">
        <.icon :if={@kind == :info} name="fa-information-circle-mini" class="h-4 w-4" />
        <.icon :if={@kind == :error} name="fa-exclamation-circle-mini" class="h-4 w-4" />
        <%= @title %>
      </p>
      <p class="mt-2 text-sm leading-5"><%= msg %></p>
      <button type="button" class="group absolute top-1 right-1 p-2" aria-label={gettext("close")}>
        <.icon name="fa-x-mark-solid" class="h-5 w-5 opacity-40 group-hover:opacity-70" />
      </button>
    </div>
    """
  end
  ...
  def flash_group(assigns) do
    ...
    <div id={@id}>
      ...
      >
        ...
        <.icon name="fa-arrow-path" class="ml-1 h-3 w-3 animate-spin" />
      </.flash>
  ...
  def error(assigns) do
    ...
    <p class="mt-3 flex gap-3 text-sm leading-6 text-rose-600 phx-no-feedback:hidden">
      <.icon name="fa-exclamation-circle-mini" class="mt-0.5 h-5 w-5 flex-none" />
      <%= render_slot(@inner_block) %>
    </p>
    """
  end
  ...
  def back(assigns) do
    ...
    <div class="mt-16">
      ...
      >
        <.icon name="fa-arrow-left-solid" class="h-3 w-3" />
        <%= render_slot(@inner_block) %>
      </.link>
    </div>
    """
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(FrankFerreiraWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(FrankFerreiraWeb.Gettext, "errors", msg, opts)
    end
  end
  ...
  def dropdown(assigns) do
    assigns =
      assigns
      |> assign_new(:options_container_id, fn ->
        "dropdown_#{:erlang.unique_integer([:positive])}"
      end)
  ...
  attr :patch, :string, default: nil, doc: "link path"
