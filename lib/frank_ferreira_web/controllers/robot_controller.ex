defmodule FrankFerreiraWeb.RobotController do
  ...
  alias FrankFerreiraWeb.RobotXML
  alias FrankFerreiraWeb.SEO
  ...
  def site_webmanifest(conn, _params) do
    json(conn, %{
      name: "frankferreira.dev",
      short_name: "Frank Ferreira",
      icons:
  ...
  def rss(conn, %{"locale" => locale} = _params) do
  ...
  def browserconfig(conn, %{"locale" => locale} = _params) do
    render_xml(conn, "browserconfig.xml", conn: conn, posts: Blog.published_posts(locale))
  end

  def sitemap(conn, %{"locale" => locale} = _params) do
    render_xml(conn, "sitemap.xml", conn: conn, posts: Blog.published_posts(locale))
  end
